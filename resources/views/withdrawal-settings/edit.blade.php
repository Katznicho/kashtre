<x-app-layout>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Current approvers data
            const businessApprovers = @json($withdrawalSetting->businessApprovers);
            const kashtreApprovers = @json($withdrawalSetting->kashtreApprovers);
            
            // Function to create approver selection dropdown
            function createApproverSelect(container, level, index, selectedValue = '') {
                ...
                users.forEach(user => {
                    ...
                    if (selectedValue === `user:${user.id}`) option.selected = true;
                ...
                if (currentBusinessId !== 1) {
                    ...
                    contractors.forEach(contractor => {
                        ...
                        if (selectedValue === `contractor:${contractor.id}`) option.selected = true;
            ...
            // Load existing business approvers
            businessApprovers.forEach(approver => {
                const value = `${approver.approver_type}:${approver.approver_id}`;
                createApproverSelect(businessApproversContainer, 'business', businessApproverCount, value);
            });
            
            // Load existing kashtre approvers
            kashtreApprovers.forEach(approver => {
                const value = `${approver.approver_type}:${approver.approver_id}`;
                createApproverSelect(kashtreApproversContainer, 'kashtre', kashtreApproverCount, value);
            });
            
            // If no existing approvers, add minimum required
            if (businessApproverCount === 0) {
