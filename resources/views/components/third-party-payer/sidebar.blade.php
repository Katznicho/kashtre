<div class="min-w-fit">
    ...
    <div id="sidebar" class="flex flex-col absolute z-40 left-0 top-0 lg:static lg:left-auto lg:top-auto lg:translate-x-0 h-[100dvh] overflow-y-scroll lg:overflow-y-auto no-scrollbar w-64 lg:w-20 lg:sidebar-expanded:!w-64 2xl:!w-64 shrink-0 bg-white dark:bg-gray-800 p-4 transition-all duration-200 ease-in-out rounded-r-2xl shadow-xs" :class="sidebarOpen ? 'max-lg:translate-x-0' : 'max-lg:-translate-x-64'" @click.outside="sidebarOpen = false" @keydown.escape.window="sidebarOpen = false">
        ...
        <div class="flex justify-between mb-10 pr-3 sm:px-2">
            ...
            <!-- Logo and Third-Party Payer Info -->
            <div class="flex flex-col items-center w-full">
                ...
                @php
                    ...
                    $thirdPartyPayer = $account ? $account->thirdPartyPayer : null;
                @endphp
                @if($thirdPartyPayer)
                <h2 class="text-[#011478] font-bold text-sm mt-2 text-center">
                    {{ $thirdPartyPayer->name }}
                </h2>
        ...
        <div class="space-y-8">
            <div>
                <ul class="mt-3 space-y-2">
                    ...
                    <!-- Dashboard -->
                    <li>
                        <a href="{{ route('third-party-payer-dashboard.index') }}" class="flex items-center pl-4 pr-3 py-2 rounded-lg {{ request()->routeIs('third-party-payer-dashboard.index') ? 'bg-blue-100 text-blue-900 font-semibold' : 'text-gray-700 hover:text-blue-700 hover:bg-blue-50' }}">
                    ...
                    <!-- Balance Statement -->
                    <li>
                        <a href="{{ route('third-party-payer-dashboard.balance-statement') }}" class="flex items-center pl-4 pr-3 py-2 rounded-lg {{ request()->routeIs('third-party-payer-dashboard.balance-statement') ? 'bg-blue-100 text-blue-900 font-semibold' : 'text-gray-700 hover:text-blue-700 hover:bg-blue-50' }}">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1"></path>
                            </svg>
                            <span class="text-sm font-medium ml-3 lg:opacity-0 lg:sidebar-expanded:opacity-100 2xl:opacity-100 duration-200">Balance Statement</span>
                        </a>
                    </li>
        ...
        <div class="pt-4 mt-auto">
            <form method="POST" action="{{ route('third-party-payer.logout') }}">
                @csrf
