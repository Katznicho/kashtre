    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mt-8">
            <div class="bg-white shadow sm:rounded-lg">
                <form action="{{ route('bank-schedules.update', $bankSchedule) }}" method="POST" class="px-4 py-5 sm:p-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        ...
                        <div>
                            ...
                            <select name="business_id" id="business_id" required
                                ...
                                @foreach($businesses as $business)
                                    <option value="{{ $business->id }}" {{ (old('business_id', $bankSchedule->business_id) == $business->id) ? 'selected' : '' }}>
                                        {{ $business->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="text" name="client_name" id="client_name" required
                                   value="{{ old('client_name', $bankSchedule->client_name) }}"
                        ...
                        <div>
                            ...
                            <input type="number" name="amount" id="amount" required step="0.01" min="0.01"
                                   value="{{ old('amount', $bankSchedule->amount) }}"
                        ...
                        <div>
                            ...
                            <input type="text" name="bank_name" id="bank_name" required
                                   value="{{ old('bank_name', $bankSchedule->bank_name) }}"
                        ...
                        <div>
                            ...
                            <input type="text" name="bank_account" id="bank_account" required
                                   value="{{ old('bank_account', $bankSchedule->bank_account) }}"
                        ...
                        <div>
                            ...
                            <select name="withdrawal_request_id" id="withdrawal_request_id"
                                ...
                                @foreach($withdrawalRequests as $request)
                                    <option value="{{ $request->id }}" {{ (old('withdrawal_request_id', $bankSchedule->withdrawal_request_id) == $request->id) ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <input type="text" name="reference_id" id="reference_id"
                                   value="{{ old('reference_id', $bankSchedule->reference_id) }}"
                        ...
                        <div>
                            ...
                            <select name="status" id="status" required
                                ...
                                <option value="pending" {{ old('status', $bankSchedule->status) == 'pending' ? 'selected' : '' }}>Pending</option>
                                <option value="processed" {{ old('status', $bankSchedule->status) == 'processed' ? 'selected' : '' }}>Processed</option>
                                <option value="cancelled" {{ old('status', $bankSchedule->status) == 'cancelled' ? 'selected' : '' }}>Cancelled</option>
                            </select>
                            @error('status')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
