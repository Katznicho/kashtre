...
function paPanel() {
    return {
        expanded: false,
        paState: 'idle',        // idle | broadcasting | busy
        selectedSectionId: '',
        selectedSectionName: '',
        busyAnnouncer: '',
        errorMsg: '',
        ...
        init() {
            ...
            this._sections = @json($paSections->pluck('name', 'id'));
            this.pollStatus();
        },

        pollStatus() {
            fetch('{{ route('pa.status') }}', { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
                .then(r => r.json())
                .then(data => {
                    if (data.active && !data.is_mine) {
                        this.paState = 'busy';
                        this.busyAnnouncer = data.announcer_name;
                    } else if (!data.active && this.paState !== 'broadcasting') {
                        this.paState = 'idle';
                    }
                })
                .catch(() => {});
            // Re-poll every 10s (Reverb events also update state in real time)
            setTimeout(() => this.pollStatus(), 10000);
        },

        get selectedSectionName() {
            return this._sections ? (this._sections[this.selectedSectionId] || '') : '';
        },
        ...
        async startAnnouncement() {
            ...
            // 2. Lock PA on server
            const res = await fetch('{{ route('pa.start') }}', {
                method: 'POST',
            ...
            this._recorder.addEventListener('dataavailable', async (e) => {
            ...
            this._recorder.start(500); // 500ms chunks
        },
