        ...
        <div class="md:flex md:items-center md:justify-between mb-6">
            ...
            <form method="GET" action="{{ route('emergency.log') }}" class="mt-4 md:mt-0 flex items-center gap-2">
                ...
                @if($date !== now()->toDateString())
                    <a href="{{ route('emergency.log') }}"
                       class="text-sm text-red-600 hover:text-red-800">Today</a>
                @endif
            </form>
        </div>
                        ...
                        <tbody class="bg-white divide-y divide-gray-100">
                            @foreach($alerts as $alert)
                                @php
                                    ...
                                    $colorClass = $colorClasses[$alert->color ?? 'red'] ?? $colorClasses['red'];
                                @endphp
